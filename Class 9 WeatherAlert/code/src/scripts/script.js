let navService = {
    ...
    registerNavListeners: function() {
        ...
        this.searchBtn.addEventListener("click", function(event) {
            ...
            pagingService.resetPagingValues()
...
let pagingService = {
    ...
    registerPagingListeners: function() {
        this.pagePrev.addEventListener("click", function() {
            if (pagingService.page > 1) {
                pagingService.from -= pagingService.selectedItems
                pagingService.to -= pagingService.selectedItems
                pagingService.page--
            }
    ...
    adaptPageButtons: function() {
        console.log(pagingService.page);
    ...
    setTotalPages: function(list) {
        pagingService.totalPages = Math.ceil(list.length / pagingService.selectedItems)
    },
    createPagingButtons: function() {
        ...
        for (let i = 0; i < pagingService.totalPages; i++) {
            document.getElementById(`page${i + 1}`).addEventListener("click", function() {
                pagingService.from = i * pagingService.selectedItems;
                pagingService.to = (i + 1) * pagingService.selectedItems;
...
// Call this function on changing dropdown selected item
function selectedVal(element){
    pagingService.selectedItems = parseInt(element.options[element.selectedIndex].text);
    let tempCachedData = weatherService.cachedData;
     
    uiService.loadStatistics(tempCachedData)
    uiService.loadHourlyTable(tempCachedData, pagingService.from, pagingService.to)
    pagingService.setTotalPages(tempCachedData.list)
    pagingService.createPagingButtons()
}

let sortService = {
    temperatureSort: document.getElementById("tempHtml"),
    humiditySort: document.getElementById("humHtml"),
    windSort: document.getElementById("windHtml"),
    ...
    registerSortListeners: function() {
        ...
        sortService.temperatureSort.addEventListener("click", function () {
            sortService.sortByHumOrWindOrTemp(weatherService.cachedData, "temp");
            weatherService.getCachedData();
        });
        sortService.humiditySort.addEventListener("click", function () {
            sortService.sortByHumOrWindOrTemp(weatherService.cachedData, "humidity");
            weatherService.getCachedData();
        });
        sortService.windSort.addEventListener("click", function () {
            sortService.sortByHumOrWindOrTemp(weatherService.cachedData, "wind");
            weatherService.getCachedData();
        });
    },
// Sorting function
